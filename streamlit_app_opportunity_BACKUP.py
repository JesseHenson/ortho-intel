...
def main():
    ...
    if run_analysis and competitors:
        run_opportunity_analysis(competitors, focus_area, analysis_type)
    else:
        show_demo_dashboard()

def run_opportunity_analysis(competitors: List[str], focus_area: str, analysis_type: str):
...
def display_opportunity_results(result: Dict[str, Any], competitors: List[str], focus_area: str):
    ...
    with tab1:
        display_category_opportunities(result.get("brand_opportunities", []), "Brand Strategy")
    
    with tab2:
        display_category_opportunities(result.get("product_opportunities", []), "Product Innovation")
    
    with tab3:
        display_category_opportunities(result.get("pricing_opportunities", []), "Pricing Strategy")
    
    with tab4:
        display_category_opportunities(result.get("market_expansion_opportunities", []), "Market Expansion")
